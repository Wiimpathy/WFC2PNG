...
int PNGU_EncodeFromYCbYCr (IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, void *buffer, PNGU_u32 stride)
{
    ...
    png_set_IHDR (ctx->png_ptr, ctx->info_ptr, width, height, 8, PNG_COLOR_TYPE_RGBA, 
	...
	ctx->img_data = malloc ( (rowbytes * height) * 4);
	if (!ctx->img_data)
	{
		png_destroy_write_struct (&(ctx->png_ptr), (png_infopp)NULL);
		if (ctx->source == PNGU_SOURCE_DEVICE)
	...
	ctx->row_pointers = malloc ((sizeof (png_bytep) * height) * 4);
	if (!ctx->row_pointers)
	{
		png_destroy_write_struct (&(ctx->png_ptr), (png_infopp)NULL);
		if (ctx->source == PNGU_SOURCE_DEVICE)
	...
	png_write_png (ctx->png_ptr, ctx->info_ptr, PNG_TRANSFORM_BGR, NULL);
...
*r1 = pngu_clamp (val[0] + b, 0, 255);
	*g1 = pngu_clamp (val[0] + g, 0, 255);
	*b1 = pngu_clamp (val[0] + r, 0, 255);

	*r2 = pngu_clamp (val[2] + b, 0, 255);
	*g2 = pngu_clamp (val[2] + g, 0, 255);
	*b2 = pngu_clamp (val[2] + r, 0, 255);
...
int pngu_decode (IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, PNGU_u32 stripAlpha, int force32bit)
{
	...
	// Check file corruption. Don't abort just return an error.
